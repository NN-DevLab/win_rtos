...
| S02 | 画像取得画面 | 同一LAN内の機器を検索し、フォルダを掘ってファイルを取得する |
...
| 要素 | 種類 | 説明 |
|---|---|---|
| 機器一覧 | リスト表示エリア | 同一LAN内で見つかった機器、または選択した機器のフォルダ・ファイルを表示 |
| 機器検索 | ボタン | LAN内の機器をFTPで探索する |
...
| # | どんな時に | どんなことをしたら | どんなことをしてくれる | 対応FR |
|---|---|---|---|---|
| 1 | 画像取得画面表示中 | 「機器検索」を押す | 同一LAN内の機器をFTPで検索し、機器一覧に表示する | FR1 |
| 2 | 機器一覧に機器が表示されている時 | 機器をタッチする | その機器のフォルダ一覧を機器一覧エリアに表示する(FTP) | FR2 |
| 3 | フォルダ一覧表示中 | フォルダをタッチする | そのフォルダの中身を表示する(FTP、繰り返し掘れる) | FR2 |
| 4 | ファイルが一覧に表示されている時 | ファイルをタッチする | ファイル情報エリアに詳細を表示する | |
| 5 | ファイルを選択している時 | 「取得開始」を押す | FTPで特定したパスに対してHTTPでダウンロードし、FileXに保存する | FR3, FR4 |
| 6 | ダウンロードが成功した時 | (自動) | D02(取得結果パネル)に成功メッセージを表示する。OKを押すとS02(フォルダ一覧の状態)に戻る | FR3, FR4 |
| 7 | ダウンロードが失敗した時 | (自動) | D02(取得結果パネル)に失敗メッセージを表示する。OKを押すとS02(フォルダ一覧の状態)に戻る | |
...
| FR | 内容 | 対応する画面・操作 |
|---|---|---|
| FR1 | 同一LAN内の機器を検索・一覧表示する | S02「機器検索」 |
| FR2 | 機器を選択するとフォルダ一覧を表示し、掘って探せる | S02「機器一覧」タッチ操作 |
