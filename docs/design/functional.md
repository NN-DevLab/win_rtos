...
| # | どんな時に | どんなことをしたら | どんなことをしてくれる | 対応FR |
|---|---|---|---|---|
| 1 | 画像取得画面表示中 | 「戻るボタン」を押す | S01(メイン画面)に戻る | |
| 2 | 画像取得画面表示中 | 「接続」を押す | D06(接続先入力ダイアログ)が開く | |
| 3 | D06で「接続開始」を押した時 | (自動) | 接続状態が「接続中...」になる | FR1 |
| 4 | 接続処理が完了した時(成功) | (自動) | 接続状態が「接続済み(IPアドレス)」になり、フォルダ一覧に接続先のルートフォルダの中身を表示する | FR1 |
| 5 | 接続処理が完了した時(失敗) | (自動) | 接続状態が「接続失敗」になる | |
| 6 | フォルダ一覧表示中 | フォルダをタッチする | そのフォルダの中身を表示する(FTP、繰り返し掘れる) | FR2 |
| 7 | ファイルが一覧に表示されている時 | ファイルをタッチする | ファイル情報エリアに詳細を表示する | |
| 8 | ファイルを選択している時 | 「取得開始」を押す | FTPで特定したパスに対してHTTPでダウンロードし、FileXに保存する | FR3, FR4 |
| 9 | ダウンロードが成功した時 | (自動) | D02(取得結果パネル)に成功メッセージを表示する。OKを押すとS02(フォルダ一覧の状態)に戻る | FR3, FR4 |
| 10 | ダウンロードが失敗した時 | (自動) | D02(取得結果パネル)に失敗メッセージを表示する。OKを押すとS02(フォルダ一覧の状態)に戻る | |
