...
flowchart LR
    ...
    subgraph App["画像合成アプリ(ポコポコラージュ)"]
        direction TB
        Behavior["・LAN内の機器を探索して画像を取得する<br/>・取得した画像を永続化する<br/>・画像の閲覧・合成をGUI上で行う"]
    end
...
| FTP探索スレッド | 同一LAN内の機器・フォルダの探索(NetX Duo FTPクライアント) | 自分で作るスレッド。普段はキューで指示待ち |
...
- 対象: 機器が見つからない、接続タイムアウト、転送中の切断など
